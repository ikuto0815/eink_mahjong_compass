...
GxEPD2_BW<GxEPD2_420_GDEY042T81, GxEPD2_420_GDEY042T81::HEIGHT>
	display(GxEPD2_420_GDEY042T81(CS_PIN, DC_PIN, RES_PIN, BUSY_PIN)); // 400x300, SSD1683
	...
	for (int y = 0; y < size; y++) {
		for (int x = 0; x < size; x++) {
			if (qrcodegen_getModule(qrcode, x, y))
				display.fillRect(offset_x + (x * scale), offset_y + (y * scale), scale, scale,
						 GxEPD_BLACK);
		}
	}
}
...
void show_connect_screen(void)
{
	...
	bool ok = qrcodegen_encodeText(text, tempBuffer, qrcode, errCorLvl, qrcodegen_VERSION_MIN,
